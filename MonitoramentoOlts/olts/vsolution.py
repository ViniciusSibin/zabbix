    ...
    for oid in pon_index_oid:
        ...
        if not pon_index:
            ...
            for i in pon_index:
                ...
                queryPON = f"INSERT INTO pon (olt_id, pon_index, slot_porta, amperagem, tensao, tx_power, status, temperatura) VALUES ({olts[0]}, {i}, '{pon_nome_val}', '{pon_corrente_val}', '{pon_tensao_val}', '{pon_tx_power_val}', '{pon_status_val}', '{pon_temperatura_val}')"
                print(queryPON)
                # Executa a query SQL
                bd.execute(queryPON)

	...
	while($queryConsultada = $execQuery->fetch_assoc()){
		...
		foreach($OIDindexPon as $chavePon => $valorPon){
			...
			if(is_null($DBPonFetch)) {
				$inserirPON = "INSERT INTO pon (olt_id,slot_porta,autorizados,amperagem,tensao,tx_power,status,temperatura) VALUES ('$olt_id','$slot_porta','$autorizados','$amperagem','$tensao','$tx_power','$status','$temperatura')";

				$mysqli->query($inserirPON) or die($mysqli->error);
			} else {
				$atualizarPon = "UPDATE pon SET autorizados='$autorizados', amperagem='$amperagem', tensao='$tensao', tx_power='$tx_power', status='$status', temperatura='$temperatura' WHERE olt_id = '$olt_id' AND slot_porta = '$slot_porta'";
